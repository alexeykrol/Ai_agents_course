...
import { AirtableRecord, EditableFields, NewAgentFields } from '../types';
import { apiService } from '../services/api';

export function useRecords() {
  const [records, setRecords] = useState<AirtableRecord[]>([]);
  const [activeRecord, setActiveRecord] = useState<AirtableRecord | null>(null);
  ...
  const selectRecord = useCallback((record: AirtableRecord) => {
  ...
  const selectRecord2 = useCallback((record: AirtableRecord) => {
    setActiveRecord(record);
    setIsEditing(false);
    // Clear undo state when selecting a different record
    setLastCreatedRecordId(null);
